...
class ML_model():
    ...
    def __init__ (self, rand_state=11):
...
# 		self.X, self.y, test_size=0.2, random_state=rand_state)
    ...
    # 		sample = self.__scaled_row(sample)
        ...
        if self.model_name == "xgboost":
            ...
            x3=pd.DataFrame(sample.reshape(1, -1))
            x3.columns= self.column_names
            result = self.model.predict_proba(x3)
        else:
    ...
    def run_model_data(self, data_set):
        ...
        if self.model_name == "xgboost":
            x3=pd.DataFrame(data_set)
            x3.columns= self.column_names

            pred = self.model.predict(x3)
        else:
    ...
    def run_model_data_prob(self, data_set):
        ...
        if self.model_name == "xgboost":
            x3=pd.DataFrame(data_set)
            x3.columns= self.column_names
            pred = self.model.predict_proba(x3)
...
class external_models(ML_model):
    def train_model(self):
        self.model.fit(self.X_tr,self.y_tr.reshape(self.y_tr.shape[0],))
